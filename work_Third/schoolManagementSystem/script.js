...
function validate(){
    if(document.getElementById("doc").checkValidity()){
        if(document.getElementById("name").checkValidity()){
            if(document.getElementById("lstName").checkValidity()){
                if(document.getElementById("adress").checkValidity()){
                    if(document.getElementById("tel").checkValidity()){
                        if(document.getElementById("mail").checkValidity()){
                            if(document.getElementById("tutor").checkValidity()){
                                if(document.getElementById("celTutor").checkValidity()){
                                    register();
                                }
                            }
                        }
...
function register(){
    ...
    alert(`El estudiante ${names[cont++]} ha sido creado`);
    cont++;
    clean();
}
function clean(){
...
function addNote(){
    if(document.getElementById('note').checkValidity()){
        notes[cont]=document.getElementById('note').value;
...
function operateNotes(){
    ...
    for(let i=0;i<=notes.length;i++){
        total+=notes[i];
        definitive=parseInt(total/notes.length);
    }
    defNotes[cont]=definitive;
printDefNotes();
}
function printDefNotes(){
    let dfn=0;
    for(let i;i<defNotes.length;i++){
        defNotes[i]==dfn;
    }
